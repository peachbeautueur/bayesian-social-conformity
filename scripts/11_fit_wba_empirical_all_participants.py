"""Fit the Stan WBA model separately to all empirical participants.

This script implements a no-pooling empirical analysis for the Simonsen
dataset. Each participant is fitted separately with the existing WBA Stan
model, and a compact summary table is saved for later comparison.

Run from the project root:
    python scripts/11_fit_wba_empirical_all_participants.py
"""
...
def fit_one_participant(
    ...
    participant_output_dir = results_dir / "wba_per_participant"
    ...
    full_summary_path = participant_output_dir / f"wba_empirical_participant_{participant_id}_full_summary.csv"
    full_summary.to_csv(full_summary_path)

    return {
        "participant_id": int(participant_id),
        "n_trials": int(len(participant_df)),
        "posterior_mean_w_direct": extract_parameter_mean(full_summary, "w_direct"),
        "posterior_mean_w_social": extract_parameter_mean(full_summary, "w_social"),
...
def main() -> None:
    """Fit the WBA Stan model separately to all empirical participants."""
    ...
    print(f"Found {len(participant_ids)} participants. Starting no-pooling WBA fits...")
    ...
    for index, participant_id in enumerate(participant_ids, start=1):
        ...
        try:
            ...
            print(
                ...
                f"{participant_id}: w_direct={summary_row['posterior_mean_w_direct']:.3f}, "
                f"w_social={summary_row['posterior_mean_w_social']:.3f}, "
                f"sigma={summary_row['posterior_mean_sigma']:.3f}, "
            ...
            summary_row = {
                "participant_id": int(participant_id),
                "n_trials": int(len(participant_df)),
                "posterior_mean_w_direct": pd.NA,
                "posterior_mean_w_social": pd.NA,
    ...
    output_path = results_dir / "wba_empirical_all_participants_summary.csv"
