"""Fit the first Stan SBA model to synthetic SBA data.

This script loads the synthetic SBA dataset, prepares Stan inputs, fits the
lecture-inspired Simple Bayesian Agent model with cmdstanpy, and saves basic
fit outputs for inspection.

Run from the project root:
    python scripts/05_fit_sba_synthetic.py
"""
...
def load_synthetic_sba_data(csv_path: Path) -> pd.DataFrame:
    """Load the synthetic SBA dataset."""
    if not csv_path.exists():
        raise FileNotFoundError(f"Synthetic SBA data not found: {csv_path}")
...
def summarize_posterior(summary: pd.DataFrame) -> pd.DataFrame:
    """Extract a concise posterior summary for sigma."""
    parameter_rows = [row for row in ["sigma"] if row in summary.index]
    selected_columns = choose_available_columns(summary)

    if not parameter_rows:
        raise ValueError("Parameter row sigma was not found in the Stan summary.")
...
def main() -> None:
    """Fit the SBA Stan model to the synthetic SBA data."""
    project_root = Path(__file__).resolve().parents[1]
    data_path = project_root / "results" / "simulated_data" / "simulated_sba_data.csv"
    stan_file = project_root / "stan" / "sba_model.stan"
    results_dir = project_root / "results" / "fits"
    ensure_directories(results_dir)

    df = load_synthetic_sba_data(data_path)
    ...
    posterior_summary_path = results_dir / "sba_synthetic_posterior_summary.csv"
    full_summary_path = results_dir / "sba_synthetic_full_summary.csv"
    ...
    print("\n" + "=" * 80)
    print("Posterior summary for synthetic SBA fit")
    print("=" * 80)
